...
Future<void> whereBottomSheet(BuildContext context, HomePageCon cont) async {
  List<AutocompletePrediction> resultList = [];
// List<String> secondarytList = [];

  cont.state.locationSearchController.addListener(() async {
    final places =
        await FlutterGooglePlacesSdk('AIzaSyA8mT_fcQoFRNpBokvTjVXsuc-TB9k-leI');
                        ...
                        child: cont.state.searchLoading.value
                            ...
                            : ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  print(
                                      'length:${cont.state.placeList.length.toString()}');
                                  return cont.state.placeList.length == 0
                                      ? Container()
                                      : ListTile(
                                          title: HeadingTextWidget(title:
                                              cont.state.placeList[index]
                                                      .fullText,textColor: Colors.black,fontWeight: FontWeight.w600,fontSize: 14,),
                                    subtitle: SubHeadingTextWidget(title:
                                    cont.state.placeList[index]
                                        .secondaryText,textColor: Colors.black,fontWeight: FontWeight.normal,fontSize: 12,),
                                          onTap: () {
                                            cont.state.selectedPlace.value = cont
                                                .state
                                                .placeList[index].fullText;
                                            AppConstants.selectedState = cont
                                                .state
                                                .placeList[index].secondaryText;
                                            cont.state.fromAddressSelected
                                                .value = true;
                                            clearLists();
                                            print("selected Place" +
                                                cont.state.selectedPlace.value
                                                    .toString());
                                            Navigator.pop(context);
                                          },
                                        );
